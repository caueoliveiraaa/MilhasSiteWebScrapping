...
class Miles:
    ...
    @func.measure_time
    def scrape_maxmilhas_first_six_months() -> None:
        try:
            base_url: str = 'https://www.maxmilhas.com.br/busca-passagens-aereas/RT/'
            ending: str = '/1/0/0/EC'
            chrome_path = r'..\\DriverWeb\\chromedriver.exe'
                        ...
                        for index in range(len(DESTS_INTERNATIONAL)):
                            ...
                            try:
                                ...
                                if Miles.validate_total_budget(extracted_data_miles['valorTotal'], limit_value=vlr_limt):
                                    ...
                                    bot.send_message_to_group(JSON_DATA['channelInternacional'], BOT_MSG)
                            except:
                                print(
    ...
    @func.measure_time
    def scrape_maxmilhas_next_six_months() -> None:
        try:
            base_url: str = 'https://www.maxmilhas.com.br/busca-passagens-aereas/RT/'
            ending: str = '/1/0/0/EC'
            chrome_path = r'..\\DriverWeb\\chromedriver.exe'
                ...
                for i in range(183, 366):
                    ...
                    try:
                        for index in range(len(DESTS_INTERNATIONAL)):
                            ...
                            print('\n')
                            print(f'--> {EMOJIS["botHead"]} {count}º iteration: {date_ini_temp} to {date_return_temp}')
                            print(f'--> {EMOJIS["botHead"]} searching for origin: {cod_orig} - destination: {cod_dest}')
                            ...
                            try:
                                ...
                                print(
                                    f'--> {EMOJIS["botHead"]} Error extracting data in iteration {count}: ' + 
                                    f'{cod_orig} - {cod_dest} - {vlr_limt} - {date_ini_temp} - {date_return_temp}'
                                )
                                func.display_error()
                            finally:
...
if __name__ == '__main__':
    ...
    if str(sys.argv[1]).strip() == '0':
        Miles.scrape_maxmilhas_first_six_months()
    elif str(sys.argv[1]).strip() == '1':
        Miles.scrape_maxmilhas_next_six_months()
